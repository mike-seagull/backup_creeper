...
def rename_dates_to_epoch(backup_dir):
    """ mv config-router.home.michaelhollister.me-${DATETIME}.xml config.${EPOCH_DATETIME}.xml """
    log.debug("in convert_dates_to_epoch")
    for file in os.listdir(backup_dir):
        pattern = re.compile(r'\d{4}-\d{2}-\d{2}_\d{2}:\d{2}:\d{2}')
        for match in [pattern.search(file)]:
            if match:
                epoch = int(time.mktime(time.strptime(match.group(0), '%Y-%m-%d_%H:%M:%S')))
                os.rename(os.path.join(backup_dir, file), os.path.join(backup_dir, "config.%i.xml" % epoch))
...
def main(backup_dir):
    ...
    rename_dates_to_epoch(backup_dir)
    backups = get_backups(backup_dir)
    log.debug("backups are: %s" % backups)
    log.info("about to get the list of all backups as strings")
    backup_info = get_all_dates_from_list_of_backup_strings(r'(?<=config\.)(\d+)(?=\.xml)', backups, backup_dir)
